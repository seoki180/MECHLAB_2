...
1. **Maker 선택** — `Maker 선택` 버튼 클릭 → 팝업에서 제조사 선택
2. **Model 선택** — 드롭다운에서 모델 선택 (자동으로 Input 폴더 경로 설정)
3. **Input 폴더** — PDF 파일이 있는 폴더 확인/변경
4. **Output 폴더** — CSV 저장 위치 확인/변경
5. **EXTRACTION 실행** — 선택한 파서 실행, 실행 로그 실시간 표시
6. **COMPILATION 실행** — 모든 모델의 CSV를 하나로 통합

> 상태바 우측에 선택된 Maker / Model이 항상 표시됩니다.

---

## 파서 추가 방법

`parsers/` 폴더에 새 파일을 추가하기만 하면 자동 등록됩니다.
